...
export default function StripeStatusBanner() {
  ...
  useEffect(() => {
    if (paymentStatus || stripeStatus || paypalStatus) {
      setVisible(true)
      const timer = setTimeout(() => setVisible(false), 6000)
      return () => clearTimeout(timer)
    }
  }, [paymentStatus, stripeStatus, paypalStatus])
  ...
  if (paypalStatus === 'connected') {
    return (
      <div className="bg-green-500/10 border border-green-500/20 rounded-xl p-4 flex items-center gap-3">
        <CheckCircle size={20} className="text-green-400 shrink-0" />
        <p className="text-sm font-medium text-green-400">PayPal connected! Members can now pay via PayPal.</p>
      </div>
    )
  }
